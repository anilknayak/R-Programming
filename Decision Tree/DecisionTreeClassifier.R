...
post(decision_tree, file = "tree.ps", title = "Classification Tree for cars")

#Prining Tree
#find the minimum xerror and choose the respective CP to prune the tree
pruned_tree = prune(decision_tree,cp=decision_tree$cptable[which.min(decision_tree$cptable[,"xerror"]),"CP"])
plot(pruned_tree, uniform=TRUE,  main="Pruned Classification Tree for Cars")

text(pruned_tree, use.n=TRUE, all=TRUE, cex=.8)
post(pruned_tree, file = "pruned_tree.ps", title = "Pruned Classification Tree for Cars")

#Decision Tree using ctree function, where prune is not required
c_decision_tree <- ctree(formula, data=training_data)
plot(c_decision_tree, main="CTree : Decision Tree for Cars")

#Misclassification using Tree Library
print(summary(tree(formula, data=training_data)))
