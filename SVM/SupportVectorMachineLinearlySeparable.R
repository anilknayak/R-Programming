...
pdf("SupportVectorMachineLinearlySeparable.pdf")
...
plot(data$gvh,data$alm1,col=data$class)
...
svmfit = svm(class~.,data=t1[,col],kernel="linear", cost=0.1)
print(svmfit)
plot(svmfit,t1[,col],main="SVM Before Tuning")

print(svmfit$index)

tuned <- tune(svm,class~.,data=t1[,col],kernel="linear", range=list(cost=c(0.001,0.01,0.1,1,10,100)))
print(summary(tuned))

svmfit_tuned = svm(class~.,data=t1[,col],kernel="linear", cost=tuned$best.parameters$cost)
print(svmfit_tuned)
